...
load('Normalized_New_Data2.mat', 'Nr2');
...
% Splitting the data
train_data = Data2(indices(1:num_train), :); % Selects the first num_train indices from the indices array
test_data = Data2(indices(num_train+1:num_train+num_test), :); % Selects the next num_test indices, starting right after the training data indices.
val_data = Data2(indices(num_train+num_test+1:end), :); % Selects the remaining indices, starting right after the testing data indices until the end of the indices array.
...
save('test_data2.mat', 'test_data');
save('val_data2.mat', 'val_data');
